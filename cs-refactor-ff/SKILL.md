...
```
✓ 已完成。方法：{方法名}。改动：{文件路径:行号范围}。验证：{跑了什么测试 / 通过情况}。Review：{subagent reviewer / fresh self-review，P0/P1 无阻塞；如有记录文件则列路径}。
```

有偏离 / apply 过程中发现想再改点别的 → **停下问用户不发挥**。

汇报前按 shared-conventions 第 2.6 节做独立 code review；没有 subagent 能力时用 fresh self-review 替代并写明。若用户要求为 ff 留记录，review 证据也写进同一 refactor 目录的 `{slug}-implementation-review.md`。
