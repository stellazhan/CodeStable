...
> main 协调 / worktree 执行 / 批次后 code review 规则看 `.codestable/reference/shared-conventions.md` 第 2.6 节。ff 很轻，但改代码仍默认走独立 worktree；用户明确说直接改当前 checkout 时可以继续并在汇报里说明。
...
```
✓ 已完成。方法：{方法名}。改动：{文件路径:行号范围}。验证：{跑了什么测试 / 通过情况}。Review：{subagent reviewer / fresh self-review，P0/P1 无阻塞}。
```

有偏离 / apply 过程中发现想再改点别的 → **停下问用户不发挥**。

汇报前按 shared-conventions 第 2.6 节做独立 code review；没有 subagent 能力时用 fresh self-review 替代并写明。
