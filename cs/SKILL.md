...
`/Users/qiyuanzhan/.agents/skills` 是 CodeStable 的本机运行时部署面，不是功能分支的试验场。任何让 CodeStable 技能"本地可用"的诉求都必须走 `codestable-maintainer`，且真实 installed root 只能从远端 `main` 同步：

1. 功能分支只做 fresh-clone 验证，必要时同步到临时 installed root。
2. 只有分支合入并推送到 `origin/main` 后，才允许把 `origin/main` 同步到 `/Users/qiyuanzhan/.agents/skills`。
3. 不要把功能分支、 sibling worktree 或手工 patch 直接覆盖到 `/Users/qiyuanzhan/.agents/skills`，否则会互相覆盖并制造安装态冲突。
...
- **目标达成**：`cs-goal` 先 interview / grill 并写起点报告，再自主实现 / 验证 / 迭代，完成前用 subagent 做功能验收
...
| CodeStable 自身技能 / harness / verifier / installed copy 更新 | `codestable-maintainer`（源码分支验证；真实 `/Users/qiyuanzhan/.agents/skills` 只从 `origin/main` 同步） |
...
| `cs-goal` | L1/L2 | Missing acceptance/start state needs grill plus start reports; completion needs subagent functional acceptance; spec/public contract change or repeated blocker raises to owner-stop. |
