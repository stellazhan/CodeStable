...
3. **确认执行拓扑**——按 shared-conventions 第 2.6 节确认是否在主协调检出讨论、是否已在独立 worktree、分支 / worktree 路径、共享计划面和禁止触碰的 sibling worktree；若不在执行 worktree，先创建 / 切换到 `.codex/worktrees/{slug}` 和 `codex/{slug}` 分支；动代码前运行 `python .codestable/tools/codestable-worktree-gate.py --root . --json start --unit .codestable/issues/YYYY-MM-DD-{slug}`
...
6. **确认执行拓扑**——按 shared-conventions 第 2.6 节确认并进入独立 worktree 修复；动代码前运行 `python .codestable/tools/codestable-worktree-gate.py --root . --json start --unit .codestable/issues/YYYY-MM-DD-{slug}`；用户明确要求当前 checkout 直接做时可以继续，但要先写 `worktree-override.md` 并在汇报里写清楚 override
