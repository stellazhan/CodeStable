...
6. **确认执行拓扑与 review 授权**——按 shared-conventions 第 2.6 节确认并进入独立 worktree 修复；若当前对话还没有明确 subagent / delegation 授权，先用 `Review options` 二选一，选 1 后继续，只有平台无 subagent 能力时选 2 才能继续；动代码前运行 `python3 .codestable/tools/codestable-worktree-gate.py --root . --json start --unit .codestable/issues/YYYY-MM-DD-{slug}`；用户明确要求当前 checkout 直接做时可以继续，但要先写 `worktree-override.md` 并在汇报里写清楚 override
