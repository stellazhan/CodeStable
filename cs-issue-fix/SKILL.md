...
3. **确认执行拓扑**——按 shared-conventions 第 2.6 节确认是否在主协调检出讨论、是否创建独立 worktree、分支 / worktree 路径、共享计划面和禁止触碰的 sibling worktree
...
6. **确认执行拓扑**——按 shared-conventions 第 2.6 节决定是否在独立 worktree 修复；用户明确要求当前 checkout 直接做时可以继续，但要在汇报里写清楚
...
修复代码写完、验证清单通过后，写 `{slug}-fix-note.md` 或 commit 前，按 shared-conventions 第 2.6 节触发独立 code review。reviewer 发现 P0 / P1 时先修并复核；没有 subagent 能力时做 fresh self-review，并在修复汇报和 fix-note 里说明替代方式。
