...
```bash
python .codestable/tools/build-review-packet.py --root . --unit .codestable/features/YYYY-MM-DD-{slug} --stage quality --output /tmp/codestable-review.md \
...
阶段交接上下文生成器。它用于给下一阶段 agent、reviewer 或 human reviewer 一份轻量 handoff，不复制完整聊天历史。

```bash
python .codestable/tools/build-context-packet.py --root . --unit .codestable/features/YYYY-MM-DD-{slug} --audience handoff --output /tmp/codestable-handoff.md \
...
未显式传 `--file` 时，工具会列当前 changed files；secret-like 路径会标记为 redacted，文本中的 token / secret / api key 会被脱敏。目标长度是 10-20 行，适合在创建下一阶段 subagent 前阅读。
